...
// Set the parameters of the BGV scheme. Return the runtime of the parameters setting.
std::chrono::duration<double> 
setParameters(CryptoContext<DCRTPoly>&, usint, int plaintextModulus = 65537, usint batchSize = 16, usint multDepth = 1, usint digitSize = 30, usint dcrtBits = 60, double sigma = 3.2);

// Get the secret/true noise of a BGV ciphertext
std::pair<NativeInteger, NativeInteger> 
bgv_get_secret_noise(CryptoContext<DCRTPoly>& cc, ConstCiphertext<DCRTPoly> ciphertext, const PrivateKey<DCRTPoly> privateKey);
