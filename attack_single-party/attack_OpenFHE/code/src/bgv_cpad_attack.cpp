...
void bgv_strategy0(){
  ...
  std::cout << "\n*********************************************\n"
    ...
    while (e0.size()>1 || e0[0] == 0){
        c0 = cc->Encrypt(keyPair.publicKey, pt);   
        ++generated_ciphertexts;
        noise_and_modulus =  bgv_get_secret_noise(cc, c0, keyPair.secretKey);
    ...
    while (ciphertexts_which_noise_has_been_found<n){
        ...
        ++generated_ciphertexts;
        noise_and_modulus =  bgv_get_secret_noise(cc, c1, keyPair.secretKey);
  ...
  std::cout << "\033[1;33m[OpenFHE][BGV] " << "number of ciphertexts which absolute noise fully determined: " << "\033[0m"<< number_of_fully_identified_noises << "/"<< generated_ciphertexts << "\n";
  std::cout << "\033[1;33m[OpenFHE][BGV] " << "number of evaluations: " << "\033[0m"<< number_of_evaluations << "\n";
  std::cout << "\033[1;33m[OpenFHE][BGV] " << "number of decryptions: " << "\033[0m"<< number_of_decryptions << "\n";
 ...
 */
std::pair<NativeInteger, NativeInteger> bgv_get_secret_noise(CryptoContext<DCRTPoly>& cc, ConstCiphertext<DCRTPoly> ciphertext, const PrivateKey<DCRTPoly> privateKey) {
...
bool is_correct_noise(NativeInteger true_noise, std::vector<uint64_t>& found_noise, NativeInteger modulus, bool print){
  ...
  true_noise = true_noise / PT_MODULUS - 1;
  if(found_noise.size() != 1)
    return false;
  if(true_noise == 0 && found_noise[0]==0 ){
  ...
  if(true_noise == (found_noise[0])%PT_MODULUS){
    ...
    if(print){ 
      ...
      std::cout << "true noise: " << true_noise << " and found noise: "<< (found_noise[0])%PT_MODULUS <<"\033[39m\n";
    }
    return false;
  }
}
